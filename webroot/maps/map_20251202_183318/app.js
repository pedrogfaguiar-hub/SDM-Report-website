...
function createPopupContent(locationName, incidents) {
    ...
    if (incidents.length === 0) {
        ...
        incidents.forEach((incident, index) => {
            ...
            // Build ServiceNow link if available
            let servicenowLink = '';
            if (incident.servicenow_url) {
                servicenowLink = `<a href="${incident.servicenow_url}"
                                     target="_blank"
                                     rel="noopener noreferrer"
                                     class="servicenow-link"
                                     title="Open in ServiceNow">
                                     <i class="fas fa-external-link-alt"></i> View in ServiceNow
                                  </a>`;
            }
            ...
            content += `
                <div class="incident-item">
                    ...
                    ${servicenowLink ? `<div class="incident-link">${servicenowLink}</div>` : ''}
